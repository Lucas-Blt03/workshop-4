...
export async function simpleOnionRouter(nodeId: number) {
  ...
  onionRouter.post("/message", async (req, res) => {
    try {
      ...
      // Parse message into RSA-encrypted symmetric key and ciphertext
      // The tests in __test__/tests/utils.js extract the first 344 chars for the encrypted key
      // The encrypted symmetric key can vary in length but 344 is specified in the tests
      const rsaEncryptedSymKey = message.substring(0, 344);
      const ciphertext = message.substring(344);
